...
设置SVG标签的的属性： `fill="currentColor" width="1em" height="1em"`。这样颜色继承自父元素的`color` css属性值，大小继承自父元素的`font-size`属性值。
...
根据[该discussion](https://github.com/tailwindlabs/tailwindcss/discussions/9090)的问答，`@tailwindcss/typography`大量使用了`:where`选择器，但是部分
